...
export async function listTaxonomy(CMS_CONTENT_DIR: string, arguments_: any) {
...
export async function getTerm(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = TaxonomyToolArgsSchema.parse(arguments_);
...
export async function createTerm(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = TaxonomyToolArgsSchema.parse(arguments_);
...
export async function updateTerm(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = TaxonomyToolArgsSchema.parse(arguments_);
...
export async function deleteTerm(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = TaxonomyToolArgsSchema.parse(arguments_);
  if (!args.id) throw new McpError(ErrorCode.InvalidParams, "ID is required");
  const filePath = safePath(CMS_CONTENT_DIR, "taxonomy", args.locale, `${args.id}.md`);
