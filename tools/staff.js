...
export async function listStaff(CMS_CONTENT_DIR, arguments_) {
    ...
    try {
        ...
        const staff = await Promise.all(files
...
export async function getStaff(CMS_CONTENT_DIR, arguments_) {
    const args = StaffToolArgsSchema.parse(arguments_);
    if (!args.id)
...
export async function createStaff(CMS_CONTENT_DIR, arguments_) {
    const args = StaffToolArgsSchema.parse(arguments_);
    if (!args.payload)
...
export async function updateStaff(CMS_CONTENT_DIR, arguments_) {
    const args = StaffToolArgsSchema.parse(arguments_);
...
export async function deleteStaff(CMS_CONTENT_DIR, arguments_) {
    const args = StaffToolArgsSchema.parse(arguments_);
    if (!args.id)
        throw new McpError(ErrorCode.InvalidParams, "ID is required");
    const filePath = safePath(CMS_CONTENT_DIR, "staff", args.locale, `${args.id}.md`);
