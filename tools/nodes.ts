...
export async function listNodes(CMS_CONTENT_DIR: string, arguments_: any) {
...
export async function getNode(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = NodeToolArgsSchema.parse(arguments_);
...
export async function createNode(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = NodeToolArgsSchema.parse(arguments_);
...
export async function updateNode(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = NodeToolArgsSchema.parse(arguments_);
...
export async function deleteNode(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = NodeToolArgsSchema.parse(arguments_);
  if (!args.id) throw new McpError(ErrorCode.InvalidParams, "ID is required");
  const filePath = safePath(CMS_CONTENT_DIR, "nodes", args.locale, `${args.id}.md`);
