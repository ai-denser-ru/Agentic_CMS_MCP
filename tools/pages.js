...
export async function listPages(CMS_CONTENT_DIR, arguments_) {
...
export async function getPage(CMS_CONTENT_DIR, arguments_) {
    const args = PageToolArgsSchema.parse(arguments_);
    if (!args.id)
...
export async function createPage(CMS_CONTENT_DIR, arguments_) {
    const args = PageToolArgsSchema.parse(arguments_);
    if (!args.payload)
...
export async function updatePage(CMS_CONTENT_DIR, arguments_) {
    const args = PageToolArgsSchema.parse(arguments_);
...
export async function deletePage(CMS_CONTENT_DIR, arguments_) {
    const args = PageToolArgsSchema.parse(arguments_);
    if (!args.id)
        throw new McpError(ErrorCode.InvalidParams, "ID is required");
    const filePath = safePath(CMS_CONTENT_DIR, "pages", args.locale, `${args.id}.md`);
