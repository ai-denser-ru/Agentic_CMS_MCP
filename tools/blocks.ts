...
export async function listBlocks(CMS_CONTENT_DIR: string, arguments_: any) {
...
export async function getBlock(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = BlockToolArgsSchema.parse(arguments_);
...
export async function createBlock(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = BlockToolArgsSchema.parse(arguments_);
...
export async function updateBlock(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = BlockToolArgsSchema.parse(arguments_);
...
export async function deleteBlock(CMS_CONTENT_DIR: string, arguments_: any) {
  const args = BlockToolArgsSchema.parse(arguments_);
  if (!args.id) throw new McpError(ErrorCode.InvalidParams, "ID is required");
  const filePath = safePath(CMS_CONTENT_DIR, "blocks", args.locale, `${args.id}.md`);
