 ...
 */
export async function verifyBuild(siteDir) {
    try {
 ...
 */
export async function getGitStatus(siteDir) {
    try {
        const projectRoot = path.join(siteDir, "..");
 ...
 */
export async function rollbackChanges(siteDir) {
    try {
        const projectRoot = path.join(siteDir, "..");
 ...
 */
export async function gitCommit(siteDir, args) {
    try {
        const projectRoot = path.join(siteDir, "..");
        const { message } = args;
        if (!message)
 ...
 */
export async function gitPush(siteDir) {
    try {
        const projectRoot = path.join(siteDir, "..");
 ...
 */
export async function getPreviewUrl(siteDir) {
    try {
        const projectRoot = path.join(siteDir, "..");
        // 1. Собираем сайт
